...
`mint_root_from_launcher(parent_coin_id, metadata, owner_puzzle_hash, delegated_puzzles)` launches a
new DataLayer coin over `chia_wallet_sdk::driver::Launcher::mint_datastore`. It anchors `root_hash`
under the given owner, returning the launch conditions + the newborn `DataStore`. Taking a
`parent_coin_id` (not a full launcher) lets a DID-authorized launcher built by `dig-did` compose
here **without a `dig-did` dependency** — the edge is one-way (dig-identity → dig-merkle).
**Signing:** the parent (funding/DID) coin spend requires its own owner's `AGG_SIG_ME`.
...
- **Prove it.** The test suite keeps golden coin-spend fixtures of each released layout; every
  format change MUST include a test decoding the older golden fixtures byte-identically.
...
- **Owner-hint domain.** The owner/delegation hint-memo domain is the fixed constant
  `b"dig:datastore:owner:v1"` (defined in dig-merkle, not imported), and MUST match across every
  DIG consumer that resolves a DataLayer owner hint.
- **Dependency layer.** dig-merkle is an L0 crate: it depends ONLY on `chia-wallet-sdk` + external
  utility crates (thiserror, hex). It MUST NOT depend on any `dig-*` crate (that would deepen the
