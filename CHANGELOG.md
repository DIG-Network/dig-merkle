# Changelog

All notable changes to this project are documented here.
This project adheres to [Semantic Versioning](https://semver.org) and
[Conventional Commits](https://www.conventionalcommits.org).

## [0.9.1] - 2026-09-04

### Documentation
- Add CONTRIBUTING.md (#15)

## [0.9.0] - 2026-08-25

### Chores
- **deps:** Bump chia-wallet-sdk to 0.36 (#14)

## [0.8.0] - 2026-08-18

### Bug Fixes
- **melt:** Refuse a melt whose owner key does not control the store (#13)

## [0.7.0] - 2026-08-10

### Bug Fixes
- **hydrate:** Bind puzzle_reveal to the coin puzzle hash before trusting a spend (#12)

## [0.6.0] - 2026-08-09

### Bug Fixes
- **mint:** Refuse Owner::Custom and add mint_datastore_launch_with_kind (#11)

## [0.5.0] - 2026-08-05

### Features
- **deps:** Migrate to the chia-wallet-sdk 0.34 / chia-0.36 family

## [0.4.5] - 2026-07-22

### CI
- **lockfile:** Gate Cargo.lock own-version + --locked in ci (#9)

## [0.4.4] - 2026-07-22

### Documentation
- **read:** Record NC-9 store-id binding invariant for resolve_owner_did (#8)

## [0.4.3] - 2026-07-20

### Bug Fixes
- **lineage:** Child_lineage_proof derives parent_inner_puzzle_hash via the DL updater path (#1332) (#7)

## [0.4.2] - 2026-07-20

### Testing
- Harden lineage update + hydrate MissingHint tests (#6)

## [0.4.1] - 2026-07-20

### Bug Fixes
- Fail-closed launcher/creator coin_id binding in resolve_owner_did (#5)

## [0.4.0] - 2026-07-20

### Features
- **dig-merkle:** DataLayer ops + resolve_owner_did + launcher-hint kind split (#4)

## [0.3.0] - 2026-07-20

### Features
- Size_bucket ("sz") replaces exact-byte size + fee overflow guard + NC-9 docstring (#3)

## [0.2.0] - 2026-07-19

### Features
- Program_hash metadata + owner-DID discovery (byte-identical DataLayer mint) (#1)

## [0.1.0] - 2026-07-19

### Features
- Crate skeleton, SPEC, signing boundary, and CI gate set (U1)


