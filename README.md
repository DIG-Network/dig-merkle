...
optional label/description/bytes/size-proof and two additive DIG keys: `program_hash` (the CLVM
tree-hash of an associated program/puzzle — stored and echoed, never computed here) and `size_bucket`
(a `SizeBucket` — the store's size as a power-of-2 bucket, `k ∈ 0..=10` ↔ `2^k MB`, 1 MB..1 GB,
CLVM key `sz`). With both `None` a mint is byte-identical to a plain DataLayer store. Its
...
|---|---|---|
| `mint::mint_datastore(parent_coin, owner, root_hash, label, description, bytes, size_proof, program_hash, size_bucket, owner_ph, delegated, fee)` | **shipped** — launch a new DataLayer store anchoring a root, byte-identical to on-chain stores | owner's `AGG_SIG_ME` |
| `size::SizeBucket` (`from_exponent`/`for_byte_len`/`exponent`/`megabytes`/`byte_len`) | **shipped** — the canonical `.dig` size-bucket ladder (`k ∈ 0..=10` ↔ `2^k MB`, 1 MB..1 GB); CLVM key `sz` | — |
...
- `types` — `MerkleCoinSpend`, `Owner`, and the re-exported SDK types (`DataStore`,
  ...
  `LineageProof`, `Proof`).
- `metadata` — `DigDataStoreMetadata`, the additive `program_hash` + `size_bucket` metadata superset (shipped, SPEC §2).
